//! Integration tests against Go fixture corpora.

use std::path::PathBuf;

use dry_core::{CloneType, Config, OutputFormat, Report, analyze};
use dry_go::GoNormalizer;
...
fn analyze_fixture(name: &str, threshold: f64) -> Report {
    ...
    config.walk.extensions = vec!["go".to_owned()];
    config.output.format = OutputFormat::Json;
    let normalizer = GoNormalizer::new(config.walk.min_nodes, config.walk.min_lines);
    let roots = vec![fixture_dir(name)];
    let result = analyze(&roots, &config, &normalizer, "dry-go");
    assert!(
        result.is_ok(),
        "analyze failed: {}",
...
#[test]
fn nested_func_literal_finds_type_one() {
    assert_first_clone(
        &analyze_fixture("nested_func_literal", 0.85),
        CloneType::Type1,
        true,
    );
}
