...
`// dry-rs:ignore` vs merge tests), fix until `findings=0`, then confirm via
`./scripts/verify.sh full`. Do not re-merge CC≤5 helpers or relax thresholds /
`dry.toml` to hide findings.
