...
./scripts/verify.sh full    # default; includes deny, audit, dry-rs
./scripts/verify.sh         # same as full
```

Print failures clearly. Do **not** relax clippy thresholds or edit `dry.toml`
solely to hide findings. Fix root causes, then re-run until green.
